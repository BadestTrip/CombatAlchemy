...
> Historical implementation plan, retained for provenance. Do not replay its
> cleanup/deletion steps as new work. On 2026-09-12 the user approved the
> [pixel-art conversion](./2026-09-12-pixel-art-conversion.md), which replaces
> the old bone/part-art presentation requirements with full-body sprite frames.
> The compact skeleton is still current until that migration lands. Preserve
> gameplay, four-facing behavior, and socket contracts; use
> [Project Architecture](../../PROJECT_ARCHITECTURE.md) for current code facts.
> The new plan's scale/camera targets supersede historical visual values below.
...
**Spec:** `docs/plannings/specs/2026-08-23-compact-player-consolidation-design.md`
...
- Historical specifications and plans may name deleted files. Stale-reference scans exclude `docs/plannings/` and inspect active code, scenes, resources, and user-facing documentation.
...
Expected: no matches. Do not include `docs/plannings/` in this assertion because the approved historical spec and plan name deleted paths.
