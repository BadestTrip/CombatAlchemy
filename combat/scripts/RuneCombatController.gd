...
func _ready() -> void:
	...
	combat_input.wheel_open_requested.connect(_on_wheel_open_requested)
	combat_input.wheel_close_requested.connect(_on_wheel_close_requested)
	...
	_log_line("Rune combat ready. Press Tab to open the rune arc, Space to prepare, then aim and shoot.")
...
func _on_wheel_close_requested() -> void:
...
func _finish_combat(victory: bool) -> void:
	...
	GameManager.complete_combat(victory)
