...
## Path to the player controller that provides projectile spawn origin and throw direction.
...
func _on_drink_requested() -> void:
	...
	if not _player_potion_target.receive_potion(recipe):
		return
	_potion_mixer.take_prepared_recipe()
	_finish_prepared_potion()
...
func _on_throw_requested() -> void:
	...
	var origin := _player_combat_controller.get_throw_origin() + direction * 48.0
...
func _finish_prepared_potion() -> void:
	_potion_mixer.clear()
	_mixer_open = false
	_potion_mixer_ui.reset_view()
	_potion_mixer_ui.set_open(false)
